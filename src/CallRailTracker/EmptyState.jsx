...
export default function EmptyStatePro({ title = "No Call Details Found", subtitle = "Try a different MySQL ID or check your filters.", onBack }) {
  return (
      ...
      <div className="mx-auto max-w-xl">
        <div className="relative bg-white rounded-2xl border border-slate-200 p-8 md:p-10 text-center overflow-hidden">
          ...
          <div className="pointer-events-none absolute inset-0 -z-10">
            <div className="absolute -top-24 -left-24 h-56 w-56 rounded-full blur-3xl opacity-40"
                 style={{ background: "radial-gradient(120px 120px at 50% 50%, rgba(56,189,248,.35), transparent 60%)" }} />
            <div className="absolute -bottom-20 -right-20 h-64 w-64 rounded-full blur-3xl opacity-30"
                 style={{ background: "radial-gradient(140px 140px at 50% 50%, rgba(99,102,241,.28), transparent 65%)" }} />
          </div>

          {/* animated ring */}
          <div className="mx-auto h-28 w-28 relative animate-glowPulse">
            <div className="absolute inset-0 rounded-2xl [mask:linear-gradient(#000,#000)_content-box,linear-gradient(#000,#000)] [mask-composite:exclude] p-[2px]"
                 style={{ background: "conic-gradient(from 0deg, rgba(56,189,248,.2), rgba(99,102,241,.25), rgba(56,189,248,.2))" }} />
            <div className="absolute -inset-[2px] rounded-2xl overflow-hidden">
              <div className="absolute inset-0 rounded-2xl opacity-40 animate-spinSlow"
                   style={{ background: "conic-gradient(from 0deg, transparent, rgba(56,189,248,.25), transparent 50%)" }} />
            </div>
